...
function toNumber(value, def) {
  ...
  return Number.isFinite(convertedVal) ? convertedVal : def;
}
...
function getFibonacciNumber(index) {
  ...
  const phi = (1 - Math.sqrt(5)) / 2;
  const fibonacciNumber = (Phi ** index - phi ** index) / Math.sqrt(5);
...
function getSumOfDigits(num) {
  return String(num)
    .split('')
    .reduce((buff, number) => Number(buff) + Number(number));
}
...
function isPowerOfTwo(num) {
  return Math.trunc(Math.log2(num)) === Math.log2(num);
}
...
function getIntegerOnString(str, base) {
  ...
  return Number.isNaN(parsedFloat) ? parsedFloat : Number.parseInt(str, base);
}
